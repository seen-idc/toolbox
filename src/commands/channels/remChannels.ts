...
slashCommandData.addChannelOption((opt) =>
  ...
  opt
    .setName('selector_category')
    .addChannelType(4)
    .setDescription('The role which is gonna be kicked')
    .setRequired(true)
)
...
export default defCommand({
  ...
  interaction: async (client, interaction) => {
    ...
    if (selCategory?.type !== 'GUILD_CATEGORY')
      throw new Error('You did not provide a valid category.')
    ...
    category.children.forEach((c) => {
      if (c.deletable) {
        counter++
        c.delete(
          `Deleting all channels in category${deleteCategory ? ' and the category itself' : ''} ${
            category.name
          } (${category.id}) issued by ${interaction.user.tag} (${interaction.user.id})`
        )
      } else {
        if (!errored) {
    ...
    interaction.reply(
      `Deleted **${counter} channels**${
        deleteCategory ? ' (and the category itself)' : ''
